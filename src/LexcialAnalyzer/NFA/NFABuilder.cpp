#include "../../../header/LexcialAnalyzer/NFA/NFABuilder.h"
...
Machine* NFABuilder::build_machine(Rule rule)
{
    ...
    for(RuleElement element : elements)
    {
        if(element.isAlpha())
        {
            ...
            // Epsilon
            if(ch == '#')ch = '\0';
            stack.push_back(new Machine(ch));
            continue;
        }
        ...
        if(element.getContent() == '|')
        {
            ...
            stack.pop_back();
            stack.push_back(OrOperator::apply(m2,m1));
        }
        ...
        else if(element.getContent() == '$')
        {
            ...
            Machine* m2 = stack.back();
            stack.pop_back() ;
            stack.push_back(AndOperator::apply(m2,m1));
        }
        else if(element.getContent() == '*')
        {
    ...
    stack[0]->get_start()->set_start() ;
    stack[0]->get_end()->set_accepted() ;
