...
#include "../../../header/LexcialAnalyzer/RuleExtractor/PostfixExpressionHandler.h"
...
vector<RuleElement> PostfixExpressionHandler::toPostfix(string expression)
{
    ...
    for(int index = 0; index < (int)expression.length(); index++)
    {
        ...
        else
        {
            ...
            if(expression[index] == '\\')
            {
                if(index + 1 <= (int) expression.length() - 1)
                {
                    index++;
                    /* check epsilon after /
...
vector<RuleElement> PostfixExpressionHandler::convertToRule(string postfixStr, vector<int> operatorsPosition)
{
    ...
    for(int index = 0; index < (int) postfixStr.length(); index++)
    {
